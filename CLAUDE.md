...
**The spec is authoritative, not this file.** Read it from the spec-mcp server
(`http://localhost:8787`), project `spec-sync-toolkit`: `build-spec` and `cli-not-mcp.adr`.
Never from local files — the toolkit has no spec directory.
...
1. **stdout carries exactly one JSON object** (spec §3). `emit()` in `src/output.ts` is the
...
| `src/norms.ts`  | foundation norms (transitional defaults + pinned section hash), spec-mcp HTTP client |
...
Sort tiers, labels, `owner-hold` precedence and the merge model belong to
`foundation.dev.process` §Worker-Loop, not to this code. Until that section carries a fenced
JSON block, `src/norms.ts` holds the defaults **plus** the pinned section hash; `doctor`
reports a moved hash as a finding. Do not silently edit the defaults — move the pin with them.
